const ServicesTeam = {
    ...
    activate: async (team) => {
        const token = localStorage.getItem('token')
        const response = await fetch(`${process.env.BASE_API}/team/activate/${team}`, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            },
    ...
    teamsDirectingGroup: async () => {
        const token = localStorage.getItem('token')
        const response = await fetch(`${process.env.BASE_API}/team/directing/group`, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            },
